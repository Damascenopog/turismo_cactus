...
export function Navbar() {
  ...
  const isRocinhaPage = pathname?.includes("/rocinha");
  const isBookingPage = pathname?.includes("/agendar");
  ...
  // Dynamic Navigation Links based on active page
  let navLinks = [
  ...
  if (isRocinhaPage) {
    navLinks = [
      ...
      { href: "/rocinha#booking", label: t("nav.booking") },
      { href: "/rocinha#roteiro", label: t("nav.roteiro") },
      { href: "/rocinha#arte", label: t("nav.arte") },
      { href: "/rocinha#faq", label: t("nav.faq") },
    ];
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        {/* Brand Logo */}
        <Link href="/" className="flex items-center group" aria-label="Cactus Turismo - Início">
          <div className="flex flex-col">
            ...
            <span className="text-xs uppercase font-extrabold tracking-wider text-blue-900 dark:text-blue-300">
              {isRocinhaPage ? "Tour Rocinha • Rio" : "Experiências no Rio"}
            </span>
          </div>
        </Link>
